class Hotel < ApplicationRecord
	...
	validates :hotel_name,:hotel_location,presence: { message: "can't be blank" }, format: { with: /\A[a-zA-Z]+\z/,
    message: "only allows letters" }
    validates :hotel_description, length: { maximum: 20, message: "maximum 20 characters allowed" }
    validates :hotel_rating,presence: { message: "can't be blank" } , :inclusion => {:in => 0..5, message:"must lie in between 0 to 5"},numericality: { only_integer: true, message:"must lie in between 0 to 5" }
